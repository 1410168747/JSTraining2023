...
async function histogramFromFile(filePath: string) {
  let histogram = new Histogram();
  let data = fs.readFile(filePath, { encoding: "utf-8" });

  for await (let chunk of await data) {
    histogram.add(chunk);
  }
...
histogramFromFile("./ch01/ex08/runMelos.txt").then((histogram) => {
