...
switch (action) {
    case 'create':
        if (otherArgs.length < 2) {
            console.error('Issueタイトルと本文が必要です。');
            process.exit(1);
        }
        createIssue(otherArgs[0], otherArgs[1]);
        break;
    case 'list':
        getIssues();
        break;
    case 'close':
        if (otherArgs.length < 1) {
            console.error('Issue番号が必要です。');
            process.exit(1);
        }
        closeIssue(otherArgs[0]);
        break;
    default:
        console.error('未知のアクションです。`close`, `create`, または `list` を使用してください。');
        process.exit(1);
}