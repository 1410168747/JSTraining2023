import { wait } from "../notes/utils.js";
...
function g1() {
    ...
    // });
    return wait(1000)
        .then(() => (console.log("A"), wait(2000)))
        .then(() => (console.log("B"), wait(3000)))
        .then(() => console.log("C"))
}
...
function g2() {
    ...
    //         .then(resolve, reject);
    // });
    return (resolve, reject) => wait(1000)
        .then(() => console.log("A"))
        .then(() => wait(2000))
        .then(() => console.log("B"))
        .then(() => wait(3000))
    ...
    return fetchUser()
        .then((user) =>
            fetchUserFriends(user)
                .then((friends) => log(`${user.name} has ${friends.length} friends!`))
        )
}
